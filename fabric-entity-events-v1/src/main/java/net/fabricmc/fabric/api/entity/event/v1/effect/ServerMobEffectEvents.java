...
public final class ServerMobEffectEvents {
	...
	static {
		BEFORE_ADD.register((effectInstance, entity, ctx) -> {
			((FabricMobEffect) effectInstance.getEffect().value()).onEffectAdded(effectInstance, entity);
		});
		AFTER_ADD.register((effectInstance, entity, ctx) -> {
			((FabricMobEffect) effectInstance.getEffect().value()).onEffectStarted(effectInstance, entity);
		});
		BEFORE_REMOVE.register((effectInstance, entity, ctx) -> {
			((FabricMobEffect) effectInstance.getEffect().value()).onEffectRemoved(effectInstance, entity);
		});
	}
